...
function PANEL:Init()
	...
	self.NoCurrentMapConfigs:SetText(translate.Get("no_configs_current"))
...
function CONFIGPANEL:Init()
	...
	self.Name:SetText(translate.Get("no_config"))
...
function MAPPANEL:SetConfig(config)
	...
	if config then
		...
		self.Map:SizeToContents()
		self.Name:SetText(self:GetNoConfigName() or translate.Get("no_config"))
		self.Name:SizeToContents()
